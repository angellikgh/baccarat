...
class Baccarat extends Component {
  ...
  selectWager(wagerType) {
    ...
    var finalbet =
      parseInt(this.state.betamount, 10) + parseInt(this.state.coinAmount, 10);
    ...
    setTimeout(
      function () {
        ...
        hiddenImage.addEventListener(
          "click",
          this.flipCards.bind(this, obj, cardImage, n)
        );
  ...
  gameRules() {
    ...
    //=====================Player Rules========================
    if (
      this.state.playerFinalScore <= 5 ||
      this.state.playerFinalScore === 10
    ) {
      playerDraw = true;
    } else if (
      this.state.playerFinalScore <= 6 ||
      this.state.playerFinalScore === 7
    ) {
      playerStand = true;
    } else if (
      this.state.playerFinalScore <= 8 ||
      this.state.playerFinalScore === 9
    ) {
    ...
    setTimeout(
      function () {
        if (this.state.playerFinalScore > this.state.bankerFinalScore) {
          ...
          this.state.playerFinalScore === this.state.bankerFinalScore
        ) {
          this.setState({
            gameTied: "winner",
          });
